...
<?php
    ...
    if (isset($_REQUEST['username'])) {
        ...
        $result = $stmt->execute();
        $user_id = null; // define um valor padrão para a variável $user_id
        if ($result) {
            $user_id = $con->lastInsertId();
        }

        // Prepare and execute the insert query with parameters for the user's role
        $query = "INSERT INTO `funcao` (`nome`, `descricao`, `users_id`) 
                  VALUES ('User', 'New User', :user_id)";
        $stmt = $con->prepare($query);
